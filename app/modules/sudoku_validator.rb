...
module SudokuValidator
  ...
  def sudoku_boxes_are_valid?(sudoku_board)
    3.times do |box_row|
      3.times do |column_row|
        unless board_values_list_is_valid?(sudoku_board.minor((3 * box_row)..(2 + 3 * box_row),
                                                              (3 * column_row)..(2 + 3 * column_row)).to_a.flatten)
          return false
        end
      end
    end
